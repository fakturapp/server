...
router
  .group(() => {
    ...
    router.get('/:id/pdf', [InvoicePdf, 'handle']).use(middleware.storageQuota())
    router.get('/:id', [InvoiceShow, 'handle']).use(middleware.storageQuota())
    router.post('/', [InvoiceCreate, 'handle']).use(middleware.storageQuota())
    router
      .post('/convert-quote/:id', [InvoiceConvertQuote, 'handle'])
      .use(middleware.storageQuota())
    router.patch('/:id/status', [InvoiceUpdateStatus, 'handle']).use(middleware.storageQuota())
    router.patch('/:id/unlink-quote', [InvoiceUnlinkQuote, 'handle']).use(middleware.storageQuota())
    router.patch('/:id/comment', [InvoiceUpdateComment, 'handle']).use(middleware.storageQuota())
    router.post('/:id/duplicate', [InvoiceDuplicate, 'handle']).use(middleware.storageQuota())
    router.put('/:id', [InvoiceUpdate, 'handle']).use(middleware.storageQuota())
