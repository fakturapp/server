...
router
  .group(() => {
    ...
    router.get('/:id', [CreditNoteShow, 'handle']).use(middleware.storageQuota())
    router.post('/', [CreditNoteCreate, 'handle']).use(middleware.storageQuota())
    router
      .post('/convert-invoice/:id', [CreditNoteConvertInvoice, 'handle'])
      .use(middleware.storageQuota())
    router.patch('/:id/status', [CreditNoteUpdateStatus, 'handle']).use(middleware.storageQuota())
    router.patch('/:id/comment', [CreditNoteUpdateComment, 'handle']).use(middleware.storageQuota())
    router.get('/:id/pdf', [CreditNoteDownloadPdf, 'handle']).use(middleware.storageQuota())
